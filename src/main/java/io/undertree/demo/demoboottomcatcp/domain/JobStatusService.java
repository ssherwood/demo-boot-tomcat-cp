...
public class JobStatusService {
    ...
    public Iterable<JobStatus> findAll(Pageable pageable) {
        return jobStatusRepository.findAll(pageable);  // the problem appears to be the additional count(*) done buy the pageable request
    }

    //@Retryable(value = SQLException.class, maxAttempts = 5, backoff = @Backoff(delay = 100, multiplier = 2))
    public Iterable<JobStatus> findAllCustom() {
     ...
     * @return
     */
    public JobStatus saveOne(JobStatus jobStatus) {
    ...
    public JobStatus updateStatusEntityManager(UUID id, String status) {
